...
class VFEPageControl: UIView {
    ...
    var centerDots = 3 { // 2
        didSet {
            centerDots = max(1, centerDots) // 2
            if centerDots > maxDots { // false
                centerDots = maxDots // 0
                print("centerDots has to be equal or smaller than maxDots")
            }
            if centerDots % 2 == 0 { // 2 % 2 == 0
                centerDots += 1
